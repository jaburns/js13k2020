// Generated by track editor Unity project
const vec3 Xc0 = vec3(-120.51,20.33,239.8),Xc1 = vec3(-113.7,78.54,452.5),Xc2 = vec3(-192.95,17.99,341.16),Xc3 = vec3(-46.51,6.94,177.52);
const vec4 Xf0 = vec4(0,0.361,0,0.932),Xf1 = vec4(-0.707,0,0,0.707),Xf2 = vec4(0,0,0,1),Xf3 = vec4(0,0.423,0,0.906),Xp0 = vec4(-140.3,65,425.3,50);
vec2 Xm( vec3 p )
{
vec2 d = vec2( 10000, 0 );
if( mod( g_traceBits.x / i_BIT6, 2. ) >= 1. )
d = min2( d, sdObj1( p, 0.,0.,0.,1.,0.5,9.,-5. ,15.,30.,0.,2.,1. ) );
return d;
}
float Xt( vec3 ro, vec3 rd, float dist )
{
traceBox( ro, rd, dist, g_traceBits.x, i_BIT6, 0.,0.,0.,1.,0.5,9.,-5.,16.,1.5,31. );
return dist < 10000. ? dist : -1.;
}
