...
public class MapEditorCamera : MonoBehaviour
{
    ...
    static void triggerUpdate()
    {
        ...
        string trackDataShader = File.ReadAllText(Path.Combine(Application.dataPath, "..", "..", "src", "sdfDefs.hlsl"))
            + "\n" + GenShader( false );

        string tracksGlsl = "// Generated by track editor Unity project"
            + "\n" + GenShader( true );

        File.WriteAllText(Path.Combine(Application.dataPath, "MapEditor", "Shaders", "trackData.gen.cginc"), trackDataShader);
